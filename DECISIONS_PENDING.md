...
| 7 | Email/SMS/WhatsApp provider for customer notifications | No provider configured anywhere (no credentials in `.env`/`.env.example`) — `StubEmailProvider` proves out the full queue/worker/retry/idempotency pipeline (persists, enqueues, sends, marks SENT) without contacting a real provider; a genuinely missing `contactEmail` is the one case it can legitimately, permanently fail on | `apps/api/src/modules/notifications/infrastructure/providers/stub-email.provider.ts` (Week 2 Day 8) | Client (which provider — SendGrid/SES/Twilio/etc. — and its credentials) |
