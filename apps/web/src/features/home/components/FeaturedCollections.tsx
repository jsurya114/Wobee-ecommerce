...
export function FeaturedCollections({ collections }: { collections: Collection[] }) {
  ...
  return (
    <section className="px-4 py-10 sm:px-6">
      <h2 className="mb-5 text-center font-display text-2xl text-text-primary">Featured collections</h2>
      <div className="mx-auto grid max-w-6xl gap-4 sm:grid-cols-2 lg:grid-cols-4">
        {collections.map((collection) => (
          <Link
            key={collection.id}
            href={`/collections/${collection.slug}`}
            className="group flex flex-col justify-between gap-4 rounded-card border border-border bg-surface p-5 transition-colors hover:border-primary"
          >
            <div>
              <h3 className="font-display text-lg text-text-primary">{collection.name}</h3>
              {collection.description ? (
                <p className="mt-1 line-clamp-2 font-body text-sm text-text-secondary">{collection.description}</p>
              ) : null}
            </div>
